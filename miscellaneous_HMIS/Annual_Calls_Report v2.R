...
all_caller_data <- hmis_caller_data %>%
  ...
  group_by(NeedHouseholdId, NeedDateAdded) %>%
  mutate(
    New_Housed_Status = max(case_when(Housed_Status == "Literally Homeless" ~ 2,
                                      Housed_Status == "Not Literally Homeless" ~ 1,
                                      Housed_Status == "Unknown" ~ 0)),
    New_Housed_Status = case_when(New_Housed_Status == 2 ~ "Literally Homeless",
                                  New_Housed_Status == 1 ~ "Not Literally Homeless",
                                  TRUE ~ "Unknown")) %>%
  ungroup() %>%
  group_by(ClientUid) %>%
